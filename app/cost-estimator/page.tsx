...
const questions: Question[] = [
	...
	},
	{
		...
		options: [
			...
			{
				label:
					"Healthcare, Manufacturing, Legal, Finance, Professional Services or Non-profit",
				value: "regulated",
				riskPoints: 20,
			},
			{
				label: "Other regulated sector",
				value: "other-regulated",
				riskPoints: 15,
			},
		],
	},
	{
		...
		options: [
			...
			{
				label: "Yes, standard personal data",
				value: "standard",
				riskPoints: 10,
			},
			{
				label: "Yes, health, financial, or regulated data",
				value: "sensitive",
				riskPoints: 20,
			},
		],
	},
	{
		...
		options: [
			{
				label: "Microsoft 365 (standard licences)",
				value: "m365-standard",
				riskPoints: 5,
			},
			{
				label: "Microsoft 365 (advanced security licences)",
				value: "m365-advanced",
				riskPoints: 0,
			},
	...
	},
	{
		...
		options: [
			...
			{
				label: "Industry or regional compliance",
				value: "single",
				riskPoints: 15,
			},
			{
				label: "Multiple regulatory requirements",
				value: "multiple",
				riskPoints: 20,
			},
			{ label: "Not sure", value: "not-sure", riskPoints: 10 },
...
const CostEstimatorPage = () => {
	...
	const riskFactors = useMemo((): RiskFactor[] => {
		...
		if (formData.mfaStatus === "no") {
			factors.push({
				label: "Multi-factor authentication not enforced",
				severity: "high",
			});
		} else if (formData.mfaStatus === "partial") {
			factors.push({
				label: "Multi-factor authentication only partially enforced",
				severity: "medium",
			});
		}
		...
		if (formData.deviceManagement === "no") {
			factors.push({
				label: "Devices not centrally managed",
				severity: "high",
			});
		}
		...
		if (formData.dataType === "sensitive") {
			factors.push({
				label: "Handling sensitive/regulated data",
				severity: "high",
			});
		}

		if (
			formData.orgType === "regulated" ||
			formData.orgType === "other-regulated"
		) {
			factors.push({
				label: "Operating in a regulated industry",
				severity: "medium",
			});
		}
		...
		if (formData.complianceStatus === "multiple") {
			factors.push({
				label: "Multiple compliance requirements",
				severity: "high",
			});
		} else if (formData.complianceStatus === "single") {
			factors.push({
				label: "Compliance requirements present",
				severity: "medium",
			});
		}
	...
	const handleEmailSubmit = async () => {
		...
		const klaviyoData = {
			...
			tags: [
				...
				formData.complianceStatus !== "none"
					? "calc_compliance_required"
					: null,
		...
		try {
			...
			const [klaviyoResponse, emailResponse] = await Promise.all([
				klaviyoPromise,
				emailPromise,
			]);
	...
	const getUserCountLabel = () => {
		const option = questions[0].options.find(
			(o) => o.value === formData.userCount,
		);
		...
		<div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
			...
			<div
				className={cn(
					...
					currentStep > 0 && currentStep < 12 ? "pt-24" : "pt-30",
				)}>
					...
					{currentStep === 0 && (
						<motion.div
							...
							<p className="text-lg sm:text-xl text-tertiary max-w-2xl mb-4 leading-relaxed">
								Get an instant IT & cybersecurity cost estimate. Answer a few
								quick questions and see what organisations like yours typically
								invest to stay secure, compliant, and fully supported.
							</p>
							...
							<p className="mt-8 text-sm text-slate-500 max-w-md">
								Designed for organisations that take security and compliance
								seriously.
							</p>
						</motion.div>
					)}
						...
						<motion.div
							...
							<motion.div
								...
								<p className="text-slate-600 text-lg">
									We&apos;ve analysed your responses and prepared a personalised
									report including your risk score, recommended service level,
							...
							<div className="w-full max-w-md space-y-6">
								<div className="text-center mb-6">
									...
									<p className="text-slate-500 text-sm">
										We&apos;ll also send a copy of your full report to your
										inbox.
									</p>
								</div>
								...
								<div className="space-y-4">
									...
									<Button
										...
										disabled={
											isSubmitting || !formData.email || !formData.consent
										}
						...
						<motion.div
							...
							<div className="text-center">
								...
								<p className="text-xl text-slate-600 mb-6">
									Based on your organisation profile with {getUserCountLabel()}{" "}
									users
								</p>
							...
							<motion.div
								...
								<p className="text-slate-600 mb-4">
									Some environments require additional setup before managed
									services can begin, such as:
								</p>
								...
								<p className="text-slate-500 text-sm">
									These are scoped separately following a short technical
									assessment.
								</p>
							</motion.div>
							...
							<motion.div
								...
								<p className="text-white/80 mb-8 max-w-xl mx-auto">
									Book a short scoping call and we&apos;ll validate this
									estimate, confirm requirements, and provide a written
									proposal.
								</p>
							...
							<p className="text-center text-sm text-slate-500 max-w-2xl mx-auto">
								<strong>Disclaimer:</strong> This estimate is indicative only
								and does not constitute a formal quotation. Final pricing is
								subject to assessment and scope confirmation.
							</p>
						</motion.div>
					)}
				</AnimatePresence>
